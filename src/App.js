...
const App = () => {
  ...
  const [multiSelectOptions, setMultiSelectOptions] = useState(initialMultiSelectOptions);
  ...
  const handleAddOption = () => {
    if (newMultiSelectValue && !multiSelectOptions.includes(newMultiSelectValue)) {
            ...
            <tr key={index}>
              <td>
                <select
                  ...
                  onChange={(e) => handleSingleSelectChange(index, e.target.value)}>
                    <option value="">Select an option</option>
                      {singleSelectOptions.filter(option => !rows.some(r => r.singleSelect === option))
                        .map((option, i) => (
                  ...
                  <select 
                    multiple
                      value={row.multiSelect}
                        onChange={(e) => handleMultiSelectChange(index, Array.from(e.target.selectedOptions, option => option.value))}>
                          {multiSelectOptions.map((option, i) => (
                          <option key={i} value={option}>{option}</option>
                          ))}
                       <option value="" disabled>Add New...</option>
                    </select>
                    <input
                         type='text'
